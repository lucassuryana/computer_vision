...
class LrLogger(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        history = self.model.history.history
        lr = self.model.optimizer.learning_rate  # Access learning rate

        if isinstance(lr, tf.keras.optimizers.schedules.LearningRateSchedule):
            # If a learning rate schedule is used, calculate decayed LR
            decayed_lr = lr(tf.cast(self.model.optimizer.iterations, tf.float32)).numpy()
        else:
            # Otherwise, treat it as a static variable
            decayed_lr = lr.numpy()

        # Append the learning rate to history
