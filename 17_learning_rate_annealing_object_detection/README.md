...
## Exponential vs Step Decay
To implement the exponential decay, you can uncomment the following code in `training.py` in line 71 and 72:
```     
model, callbacks = exponential_decay(model, callbacks)
model, callbacks = step_decay(model, callbacks)
```

You can run ` python training.py --imdir ../GTSRB/Final_Training/Images` to see the effect of different annealing strategies on your training and model performances. Make sure to feed in the GTSRB dataset as the image directory, and use the Desktop to view the visualization of final training metrics.
